/**
 *  @author DaniloVlad, ntauth
 *  @brief  Harness for AES-CBC leakage analysis
 *  @see    https://github.com/DaniloVlad/OpenSSL-AES
 */

#include "aes.h"

#include <klee/klee.h>

// static size_t AesKeySize;

Message *message_init(int length)
{
   Message *ret = new Message;

   ret->body = new unsigned char[length];
   ret->length = new int;
   *ret->length = length;

   // Used string terminator to allow string methods to work
   memset(ret->body, '\0', length);

   // Initialize aes_data
   aes256_init(ret);

   return ret;
}

int aes256_init(Message *input)
{
   AES_DATA *aes_info = new AES_DATA;

   // constexpr size_t LO_BOUND = 3, HI_BOUND = 3;
   // int bits_exp;

   // // Compute symbolic range for key size
   // bits_exp = klee_range(LO_BOUND, HI_BOUND + 1, "bits_exp");

   // for (int i = LO_BOUND; i <= HI_BOUND; i++) {
   //    if (bits_exp == i)
   //       bits_exp = i;
   // }

   // Assign symbolic key size
   // AesKeySize = 1 << bits_exp;

   aes_info->key = new unsigned char[AesKeySize];
   aes_info->iv = new unsigned char[AesKeySize];

   // Point to new data
   input->aes_settings = aes_info;

   // Set to zero
   // memset(input->aes_settings->key, 0, AesKeySize);
   // memset(input->aes_settings->iv, 0, AesKeySize);

   // Get rand bytes
   // if (!RAND_bytes(input->aes_settings->key, AesKeySize) ||
   //     !RAND_bytes(input->aes_settings->iv, AesKeySize))
   // {
   //    return 1;
   // }

   // Create symbolic key and iv
   klee_make_symbolic(input->aes_settings->key, AesKeySize, "key");
   klee_make_symbolic(input->aes_settings->iv, AesKeySize, "iv");

   return 0;
}

Message *aes256_encrypt(Message *plaintext)
{
   EVP_CIPHER_CTX *enc_ctx;
   Message *encrypted_message;

   int enc_length = *(plaintext->length) +
                    (AesBlockSize - *(plaintext->length) % AesBlockSize);

   encrypted_message = message_init(enc_length);

   // Set up encryption context
   enc_ctx = EVP_CIPHER_CTX_new();

   EVP_EncryptInit(
       enc_ctx,
       EVP_aes_256_cbc(),
       plaintext->aes_settings->key,
       plaintext->aes_settings->iv);

   // Encrypt all the bytes up to but not including the last block
   if (!EVP_EncryptUpdate(
           enc_ctx,
           encrypted_message->body,
           &enc_length,
           plaintext->body,
           *plaintext->length))
   {
      EVP_CIPHER_CTX_cleanup(enc_ctx);

      return nullptr;
   }

   // Update length with the amount of bytes written
   *(encrypted_message->length) = enc_length;

   // EncryptFinal will cipher the last block + Padding
   if (!EVP_EncryptFinal_ex(enc_ctx, enc_length + encrypted_message->body, &enc_length))
   {
      EVP_CIPHER_CTX_cleanup(enc_ctx);

      return NULL;
   }

   // Add padding to length
   *(encrypted_message->length) += enc_length;

   // No errors, copy over key & iv rather than pointing to the plaintext msg
   // memcpy(encrypted_message->aes_settings->key, plaintext->aes_settings->key, AesKeySize);
   // memcpy(encrypted_message->aes_settings->iv, plaintext->aes_settings->iv, AesKeySize);

   // Free context and return encrypted message
   EVP_CIPHER_CTX_cleanup(enc_ctx);

   return encrypted_message;
}

Message *aes256_decrypt(Message *encrypted_message)
{
   EVP_CIPHER_CTX *dec_ctx;
   Message *decrypted_message;

   int dec_length = 0;

   // Initialize return message and cipher context
   decrypted_message = message_init(*encrypted_message->length);
   dec_ctx = EVP_CIPHER_CTX_new();

   EVP_DecryptInit(
       dec_ctx,
       EVP_aes_256_cbc(),
       encrypted_message->aes_settings->key,
       encrypted_message->aes_settings->iv);

   // Same as above
   if (!EVP_DecryptUpdate(
           dec_ctx,
           decrypted_message->body,
           &dec_length,
           encrypted_message->body,
           *encrypted_message->length))
   {
      EVP_CIPHER_CTX_cleanup(dec_ctx);

      return nullptr;
   }

   *(decrypted_message->length) = dec_length;

   if (!EVP_DecryptFinal_ex(
           dec_ctx,
           *decrypted_message->length + decrypted_message->body,
           &dec_length))
   {
      EVP_CIPHER_CTX_cleanup(dec_ctx);

      return nullptr;
   }

   // Auto handle padding
   *(decrypted_message->length) += dec_length;

   // Terminate string for easier use.
   *(decrypted_message->body + *decrypted_message->length) = '\0';

   // No errors, copy over key & iv rather than pointing to the encrypted msg
   memcpy(decrypted_message->aes_settings->key, encrypted_message->aes_settings->key, AesKeySize);
   memcpy(decrypted_message->aes_settings->iv, encrypted_message->aes_settings->iv, AesKeySize);

   // Free context and return decrypted message
   EVP_CIPHER_CTX_cleanup(dec_ctx);

   return decrypted_message;
}

void aes_cleanup(AES_DATA *aes_data)
{
   delete[] aes_data->iv;
   delete[] aes_data->key;
   delete aes_data;
}

void message_cleanup(Message *message)
{
   // Free message struct
   aes_cleanup(message->aes_settings);

   delete[] message->body;
   delete message->length;
   delete message;
}

int main(int argc, char** argv)
{
   // Allocate data
   unsigned char* data = new unsigned char[AesDataSize];

   // Create & init message pointer
   Message *message, *enc_msg, *dec_msg;

   // Get message to be encrypted
   message = message_init(AesDataSize);
   memcpy(message->body, data, AesDataSize);

   if (aes256_init(message))
      return 1;

   // Taint secret key and data
   klee_set_taint(1, message->aes_settings->key, AesKeySize);
   klee_set_taint(1, message->body, AesDataSize);

   // Encrypt & Decrypt
   enc_msg = aes256_encrypt(message);
   dec_msg = aes256_decrypt(enc_msg);

   return 0;
}